...
export class QuestionRepository {
  static createSurvey(surveyName: string, content: Array<Question>): Promise<Survey> {
    return QuestionModel.create({ surveyName, content } as Survey);
  }

  static updateSurvey(surveyId: string, surveyName: string, content: Array<Question>): Promise<Survey | null> {
    return QuestionModel.findByIdAndUpdate(surveyId, { surveyName, $push: { 'content' : content } }, { new: true }).exec();
  }
